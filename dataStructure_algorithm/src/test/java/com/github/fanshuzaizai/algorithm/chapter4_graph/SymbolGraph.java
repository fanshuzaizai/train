...
public class SymbolGraph {
    ...
    public SymbolGraph(List<String> lines, String separator) {
        ...
        lines.forEach(e -> {
            ...
            if(split[0].equalsIgnoreCase("Tin Men (1987)")){
                Integer integer = st.get("DeBoy, David");
                String select = st.select(40185);

                String select2 = st.select(0);
                Integer get = st.get(select2);
